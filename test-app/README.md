...
This repository is lightweight (no external runtime dependencies) and intended as an educational/demo CLI app you can run and extend.

## Setup Instructions
1. Ensure you have Node.js 18+ installed (package.json specifies "engines": ">=18.0.0").
   - Download from: https://nodejs.org/

2. Clone the repository (replace with your repo URL if different):
...
- index.js — Main entry point. Loads questions, manages main loop, and coordinates user interaction, displaying banners and results.
...
Source:
- src/
  - colors.js — Small utility for terminal colorization using ANSI escape codes. Exports helper functions (cyan, dim, error, success, highlight, etc.).
  - input.js — Readline-based input utilities:
    - createInterface()
    - prompt(rl, question)
    - select(rl, question, options) — shows numbered options and reads numeric selection.
    - confirm(rl, question) — y/n prompt.
    - pressEnter(rl, message) — waits for Enter.
  - quiz.js — Quiz class that contains game logic:
    - shuffle() helper
    - Quiz class with methods:
      - askQuestion(rl) — displays question, records answer, shows feedback
      - showResults() — shows score summary and incorrect answers
      - renderProgressBar() — visual progress bar

Example tree (concise):
...
- Runtime: Node.js >= 18 (uses node: builtin import specifiers and fs/promises API)
...
If you want, I can:
- Generate a CONTRIBUTING.md and basic test scaffold,
- Add a small script to validate questions.json schema,
- Create a short GitHub Actions workflow to run tests or lint. Which would you like next?
